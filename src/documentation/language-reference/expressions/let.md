...
Let expressions allow one to introduce local variables.
...
{%ard%} e[z.1/x,z.2/y] {%endard%} if the type of {%ard%} z {%endard%} is a Sigma type (and with fields instead of projections
{%ard%} z.1 {%endard%} and {%ard%} z.2 {%endard%} in case of a record). In general, if pattern {%ard%} p_i {%endard%} contains
variables {%ard%} x_i^1, ... x_i^{n_i} {%endard%}, the expression 
{%ard%} \let | p_1 => e_1 ... | p_n => e_n \in e {%endard%} evaluates to {%ard%} e[... proj_i^j(e_i)/x_i^j ...] {%endard%},
where {%ard%} proj_i^j {%endard%} is the sequence of projections and field access expressions, corresponding to the {%ard%} j {%endard%}-th variable of {%ard%} p_i {%endard%}. 
