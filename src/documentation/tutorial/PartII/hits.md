...
Another important remark: if {%ard%}f : A -> B{%endard%} is a "bijection" between types, namely, satisfies {%ard%}isBij{%endard%}
...
Alternatively, we can look at the type of a space as being generated by the normal constructors, called _point-constructors_,
...
and the path-constructor {%ard%}loop : base = base{%endard%}, the 2-sphere generated by {%ard%}base{%endard%} and
...
\func pi1Mult {X : \1-Type} {x : X} (a b : pi1 X x) => a * b
{%endarend%}
